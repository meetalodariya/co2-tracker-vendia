...
const { getAllBatteriesController } = require('../get-all-battries');
...
describe('Get All batteries controller', () => {
  test('should return correct records', async () => {
    ...
    const vendiaClient = {
      entities: { battery: { list: listAllBattriesMock } },
    };
    ...
    await getAllBatteriesController(req, res, () => undefined);
  ...
  test('should return error when query to vendia fails', async () => {
    ...
    await getAllBatteriesController(req, res, nextMock);
